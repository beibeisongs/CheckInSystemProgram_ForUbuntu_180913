...
def select_course(class_name, name):
    ...
    response = insert_student(c, name, course_name, teacher_name, op1, conn)
...
def t_checkin_arouse(lng, lat, course_name, teacher_name):
    op3 = "arouse"
    create_arouse_table(c, teacher_name, course_name, op3, conn)
    insert_arouse_table(c, teacher_name, course_name, op3, lng, lat, conn)

    return "Success !"


@app.route('/checkin_photo_upload1/<class_name>/<name>/<lng>/<lat>', methods=['POST'])
def u_checkin_upload1(class_name, name, lng, lat):
    ...
    op3 = "arouse"
    result_get_lnglat, t_lng, t_lat = get_lng_lat_arouse(c, teacher_name, course_name, op3, conn)
...
def u_checkin_upload2(class_name, name):
    ...
    if result == "Good !":
        ...
        if os.path.exists(mark_path):
            ...
            op2 = "checked"
            insert_student(c, name, course_name, teacher_name, op2, conn)
